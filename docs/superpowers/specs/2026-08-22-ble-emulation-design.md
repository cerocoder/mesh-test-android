...
Приложение для работы с нодой Meshtastic под Android. 
Нужен способ отлаживать логику приложения на эмулируемом устройстве, а также реальный
BLE-транспорт для работы с настоящей нодой, которая может использоваться при ручном тестировании.
...
2. **Реальный BLE-транспорт** — сканирование, бондинг, GATT-обмен, реконнект, подключаемся к реальной ноде.
