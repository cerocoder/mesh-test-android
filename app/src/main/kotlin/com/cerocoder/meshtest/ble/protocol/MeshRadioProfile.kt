        ...
        drainTriggers.collect {
            ...
            while (keepReading) {
                val packet = try {
                    ...
                    // Узкий перехват — это классификатор, а не недосмотр.
                    // IOException считаем транзиентным: прекращаем текущий проход,
                    // но поток остаётся живым и продолжит со следующего триггера.
                    // Всё остальное (обрыв связи, отозванное разрешение, мёртвый
                    // GATT) обязано убить поток: только так транспорт узнаёт, что
                    // сессия кончилась, и запускает переподключение. Проглотив их,
                    // мы получили бы цикл, вечно ждущий триггеров от мёртвой ноды.
